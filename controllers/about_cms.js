...
function mergeSection(existing = {}, incoming = {}) {
  ...
  if (incoming.button && typeof incoming.button === "object") {
    merged.button = { ...(existing.button || {}), ...incoming.button };
  }
...
function applySectionVisibility(payload = {}) {
  ...
  if (data.ourStorySection?.button && data.ourStorySection.button.isVisible === undefined) {
    data.ourStorySection = {
      ...data.ourStorySection,
      button: { ...data.ourStorySection.button, isVisible: true },
    };
  }
...
module.exports = {
  create: async (req, res) => {
    try {
      const doc = new AboutCms(applySectionVisibility(req.body));
  ...
  getAll: async (_req, res) => {
    try {
      const docs = await AboutCms.find({ del_status: "Live" }).sort({ created_at: -1 });
  ...
  getById: async (req, res) => {
    try {
      ...
      const doc = await AboutCms.findOne({ _id: id, del_status: "Live" });
