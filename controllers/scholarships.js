...
module.exports = {
  create: async (req, res) => {
    try {
      const doc = new Scholarships(req.body);
  ...
  getAll: async (req, res) => {
    try {
      ...
      pagination.data = await Scholarships.find(filter).sort({ created_at: -1 }).skip(skip).limit(pagination.pageSize);
  ...
  getById: async (req, res) => {
    try {
      ...
      const doc = await Scholarships.findOne({ _id: id, del_status: "Live" });
