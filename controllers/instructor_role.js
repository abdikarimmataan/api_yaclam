...
module.exports = {
  create: async (req, res) => {
    try {
      const doc = new InstructorRole(req.body);
  ...
  getAll: async (req, res) => {
    try {
      ...
      const total = await InstructorRole.countDocuments(filter);
      ...
      pagination.data = await InstructorRole.find(filter)
        .sort({ created_at: -1 })
        .skip(skip)
  ...
  getById: async (req, res) => {
    try {
      ...
      const doc = await InstructorRole.findOne({ _id: id, del_status: "Live" });
