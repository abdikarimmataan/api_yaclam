...
module.exports = {
  create: async (req, res) => {
    try {
      const doc = new Register(req.body);
  ...
  getAll: async (req, res) => {
    try {
      ...
      pagination.data = await Register.find(filter).sort({ created_at: -1 }).skip(skip).limit(pagination.pageSize);
  ...
  getById: async (req, res) => {
    try {
      ...
      const doc = await Register.findOne({ _id: id, del_status: "Live" });
