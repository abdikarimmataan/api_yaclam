...
module.exports = {
  create: async (req, res) => {
    try {
      ...
      const doc = new UniversityCategory({
  ...
  getAll: async (req, res) => {
    try {
      ...
      const total = await UniversityCategory.countDocuments(filter);
      ...
      pagination.data = await UniversityCategory.find(filter)
        .sort({ name: 1 })
        .skip(skip)
  ...
  getById: async (req, res) => {
    try {
      ...
      const doc = await UniversityCategory.findOne({ _id: id, del_status: "Live" });
  ...
  updateStatus: async (req, res) => {
    try {
      ...
      const doc = await UniversityCategory.findOneAndUpdate(
