...
const Roadmaps = require("../models/roadmaps.model");
...
module.exports = {
  create: async (req, res) => {
    try {
      const doc = new Roadmaps(req.body);
  ...
  getAll: async (req, res) => {
    try {
      ...
      const total = await Roadmaps.countDocuments(filter);
      ...
      pagination.data = await Roadmaps.find(filter).sort({ created_at: -1 }).skip(skip).limit(pagination.pageSize);
  ...
  getById: async (req, res) => {
    try {
      ...
      const doc = await Roadmaps.findOne({ _id: id, del_status: "Live" });
