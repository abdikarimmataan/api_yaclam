...
module.exports = {
  ...
  getAll: async (req, res) => {
    try {
      ...
      pagination.data = await Login.find(filter).sort({ created_at: -1 }).skip(skip).limit(pagination.pageSize);
  ...
  getById: async (req, res) => {
    try {
      ...
      const doc = await Login.findOne({ _id: id, del_status: "Live" });
